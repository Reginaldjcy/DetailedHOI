"""
姿态估计模块
包含人脸姿态估计和身体姿态估计两个类， 现在是输出3个关键点（头部中心、左手、右手），后续可以根据需要调整输出的关键点数量和类型
"""
import torch
        ...
        for i in range(batch_size):
            ...
            if len(human_indices) > 0:
                ...
                for j in range(num_valid):
                    keypoints[human_indices[j]] = pred_keypoints[j]
            region_props[i]['keypoints'] = keypoints          # [N, 17, 3] 原始保留（可选）
        return region_props