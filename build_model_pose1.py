...
# 导入骨架插值函数（文件名改成你实际的模型文件名）
from keypoint_utils import attach_body_keypoints_to_region_props
...
class BodyPoseEstimator:
    ...
    def __call__(self, region_props, images):
        """
        Args:
            region_props: list of dicts, 每个dict包含'boxes'和'labels'
            images: list of tensors, 每个tensor shape为 [C, H, W]
        
        Returns:
            region_props: list of dicts, 每个dict添加了以下两个字段:
                - 'keypoints':      [num_box, 17, 3]  原始17个COCO关键点（像素坐标）
                - 'body_keypoints': [num_box, TOTAL_KEYPOINTS, 3]  17个原始点 + 骨架插值点
        """
        batch_size = len(images)

        # ── Step 1: 用 RTMPose 推理原始17个关键点 ──────────────────────────────
        # 同时构建 body_result 供 attach 函数使用
        body_result = {}  # dict[image_idx -> list[body_dict]]

        for i in range(batch_size):
            image  = images[i]          # [C, H, W]
            boxes  = region_props[i]['boxes']   # [num_box, 4]
            labels = region_props[i]['labels']  # [num_box]
            ...
            bodies_this_image = []  # 用于构建 body_result

            if len(human_indices) > 0:
                human_boxes = boxes[human_indices]  # [num_human, 4]

                with torch.no_grad():
                    pred_keypoints = self.pose_model.infer(image, human_boxes)
                    # pred_keypoints: [K, 17, 3]，K 可能 < num_human

                num_valid = len(pred_keypoints)  # 实际返回数量

                # 只赋值实际有结果的部分
                valid_human_indices = human_indices[:num_valid]
                keypoints[valid_human_indices] = pred_keypoints[:num_valid]

                # 构建 body_result 格式供 attach 函数使用（只用有效结果）
                for j in range(num_valid):
                    hid      = human_indices[j]
                    box_xyxy = boxes[hid].cpu().tolist()
                    kpts     = pred_keypoints[j].cpu().tolist()  # [[x,y,score], ...]
                    bodies_this_image.append({
                        "bbox":      box_xyxy,
                        "conf":      1.0,
                        "keypoints": kpts,
                    })
            ...
            # 写入原始关键点
            region_props[i]['keypoints'] = keypoints
            body_result[i] = bodies_this_image

        # ── Step 2: 在原始17点基础上插值，生成 body_keypoints ─────────────────
        attach_body_keypoints_to_region_props(
            region_props, body_result, human_idx=self.human_idx
        )
