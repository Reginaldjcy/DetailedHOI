...
CJ
ho_q:                            
q_pos: skeleton (3) / object - n1:n2 - n1+n2=512
"""
...
TOTAL_KEYPOINTS = 3
        ...
        # Add this method to KeypointEstimator class
    def extract_head_and_hands(self, keypoints: torch.Tensor) -> torch.Tensor:
        """
        从17个COCO关键点中提取3个关键点：头部中心、左手、右手
        
        Args:
            keypoints: torch.Tensor [N, 17, 3] (x, y, confidence)
        Returns:
            torch.Tensor [N, 3, 3]
                - [N, 0, :] 头部中心 (鼻+双眼+双耳的均值，仅用置信度>阈值的点)
                - [N, 1, :] 左腕 (index 9)
                - [N, 2, :] 右腕 (index 10)
        """
        HEAD_INDICES = [0, 1, 2, 3, 4]  # 鼻, 左眼, 右眼, 左耳, 右耳
        LEFT_WRIST  = 9
        RIGHT_WRIST = 10

        N = keypoints.shape[0]
        result = torch.zeros(N, 3, 3, device=keypoints.device)

        # --- 头部中心 ---
        head_kpts = keypoints[:, HEAD_INDICES, :]  # [N, 5, 3]
        conf_mask = head_kpts[:, :, 2] > self.kpt_conf_thres  # [N, 5] bool

        for n in range(N):
            valid = conf_mask[n]  # [5] bool
            if valid.any():
                valid_pts = head_kpts[n][valid]          # [k, 3]
                result[n, 0, :2] = valid_pts[:, :2].mean(dim=0)   # x, y 均值
                result[n, 0, 2]  = valid_pts[:, 2].mean()          # confidence 均值
            # else: 保持全0

        # --- 左腕 / 右腕 ---
        result[:, 1, :] = keypoints[:, LEFT_WRIST,  :]
        result[:, 2, :] = keypoints[:, RIGHT_WRIST, :]
...
class HumanObjectMatcher(nn.Module):
    ...
    def compute_box_pe(self, boxes, embeds, body_kpts, image_size):
        """
        body_kpts: [N, TOTAL_KEYPOINTS, 3]  (17个关键点，与kpts_pe_proj一致)
        returns:
            box_pe:      [N, 512]
        ...
        kpts_conf = body_kpts[..., 2]          # [N, TOTAL_KEYPOINTS]
        ...
        # ── 投影到各自目标维度 ──
        # Bug2修复：kpts_pe_proj 只在这里调用一次，forward 中不再重复
        kpts_pe_out = self.kpts_pe_proj(kpts_pe.reshape(N, -1))   # [N, human_centre_dim]
        # Bug1修复：使用补充定义的 obj_c_pe_proj
